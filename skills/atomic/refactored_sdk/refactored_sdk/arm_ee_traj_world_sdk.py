# -*- coding: utf-8 -*-
"""Atomic skill: arm_ee_traj_world_sdk.

Aligns with `test_arm_ee_traj_world.py` by calling `hardware.send_arm_ee_traj_sdk(left_traj, right_traj, total_time, frame='world')`.
"""
...
@dataclass
class ArmEETrajWorldSdkParams(SkillParams):
    """对齐 test_arm_ee_traj_world.py：hardware.send_arm_ee_traj_sdk(frame='world')。"""

    skill_name: str = "arm_ee_traj_world_sdk"
...
@define_manifest(
    ...
    description="对齐 test_arm_ee_traj_world.py：调用 hardware.send_arm_ee_traj_sdk(frame='world')",
    params=[
        {
            "name": "left_traj",
            "type": "json",
...
class ArmEETrajWorldSdkSkill(SkillBase):
    """手臂末端轨迹（世界系，SDK 直调）：hardware.send_arm_ee_traj_sdk(frame='world')。"""

    def __init__(self, hardware: IHardware):
        super().__init__(name="arm_ee_traj_world_sdk")
        self.hardware = hardware
        self.params: Optional[ArmEETrajWorldSdkParams] = None
        self._done = False
    ...
    def on_initialize(self, params: ArmEETrajWorldSdkParams) -> Result:
        if not isinstance(params, ArmEETrajWorldSdkParams):
            return Result.fail("Invalid parameters for ArmEETrajWorldSdkSkill")
    ...
    def on_execute(self) -> Result:
        if self._done:
            return Result.ok("ArmEETrajWorldSdkSkill already finished")
        ...
        if result.success:
            logger.info(
                "arm_ee_traj_world_sdk: left_points=%d right_points=%d total_time=%.3fs",
