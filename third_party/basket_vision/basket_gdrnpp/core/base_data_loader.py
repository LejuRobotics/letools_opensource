...
class Base_DatasetFromList(data.Dataset):
    ...
    def _bg_img_paths(self):
        ...
        if bg_type == "coco":
            ...
            img_paths = [osp.join(VOC_root, "JPEGImages/bg/{}.jpg".format(bg_idx)) for bg_idx in VOC_bg_list]
        elif bg_type == "VOC":
            VOC_root = bg_root  # path to "VOCdevkit/VOC2012"
            img_paths = [
                osp.join(VOC_root, "JPEGImages/bg", fn.name)
                for fn in os.scandir(osp.join(bg_root, "JPEGImages/bg"))
                if ".jpg" in fn.name
            ]
        elif bg_type == "SUN2012":
            img_paths = [
                osp.join(bg_root, "JPEGImages/bg", fn.name)
                for fn in os.scandir(osp.join(bg_root, "JPEGImages/bg"))
                if ".jpg" in fn.name
            ]
