...
class GDRN_DoubleMask(nn.Module):
    ...
    def forward(
        ...
        if not do_loss:  # test
            ...
            if cfg.TEST.USE_PNP or cfg.TEST.SAVE_RESULTS_ONLY or cfg.TEST.USE_DEPTH_REFINE or use_demo:
            ...
            # also keep the raw head outputs for visualization during training
            out_dict = {
                "mask":        vis_mask,
                "full_mask":   full_mask,
                "coor_x":      coor_x,
    ...
    def gdrn_loss(
        ...
        if not g_head_cfg.FREEZE:
            ...
            gt_mask_xyz = gt_masks[loss_cfg.XYZ_LOSS_MASK_GT] # visib
            if xyz_loss_type == "L1": # here
        ...
        if not g_head_cfg.FREEZE:
            ...
            gt_mask = gt_masks[loss_cfg.MASK_LOSS_GT] # trunc
            if mask_loss_type == "L1": # here
            ...
            loss_dict["loss_mask"] *= loss_cfg.MASK_LW    # times 1
        ...
        if not g_head_cfg.FREEZE and loss_cfg.FULL_MASK_LW > 0:
            ...
            if full_mask_loss_type == "L1": # here
                ...
                raise NotImplementedError(f"unknown mask loss type: {mask_loss_type}")
            loss_dict["loss_mask_full"] *= loss_cfg.FULL_MASK_LW    # times 1

        # roi region loss --------------------
        if not g_head_cfg.FREEZE:
            region_loss_type = loss_cfg.REGION_LOSS_TYPE # CE
            gt_mask_region = gt_masks[loss_cfg.REGION_LOSS_MASK_GT] # visib
            ...
            loss_dict["loss_region"] *= loss_cfg.REGION_LW    # times 1
