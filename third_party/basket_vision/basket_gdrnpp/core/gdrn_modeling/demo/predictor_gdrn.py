...
# PROJ_ROOT = "/media/qin/PortableSSD/deliver/est"
...
class GdrnPredictor():
    def __init__(self,
        ...
        #              2:'class_name_2'}
                #set your trained object names
        self.objs = ref.basket.id2obj # TODO: make more general
        self.cls_names = [i for i in self.objs.values()]
        self.obj_ids = [i for i in self.objs.keys()]
        self.extents = self._get_extents()

        # Camera intrinsics / depth scale: optional global json, may be overridden per frame
        self.cam = None
        ...
        if camera_json_path is not None:
            try:
                ...
                if all(k in camera_json for k in ('fx', 'fy', 'cx', 'cy')):
                    self.cam = np.asarray([
                ...
                self.depth_scale = float(camera_json.get('depth_scale', 1.0))
            except FileNotFoundError:
                self.cam = np.eye(3, dtype=np.float32)
                self.depth_scale = 1.0
        if self.cam is None:
            # Fallback when no camera file or incomplete json
            self.cam = np.eye(3, dtype=np.float32)
        ...
        if self.cfg.TEST.USE_DEPTH_REFINE:
            ...
            # print(f"Loaded {len(self.ren_models)} models for depth refinement.")
    ...
    def inference(self, data_dict):
        ...
        with inference_context(self.model), torch.no_grad():
            with autocast(device_type='cuda', enabled=False):  # gdrn amp_test seems slower
                out_dict = self.model(
                    data_dict["roi_img"],
        ...
        # print(f"[Predictor] GPU peak dynamic memory: {peak_mb:.1f} MB")
    ...
    def gdrn_visualization(
        ...
        - Predicted full mask (left, red)
        - Pose-projected mask from predicted pose (right, yellow)
        - If GT pose provided, add top row with GT mask (blue) and GT pose mask (yellow)
        Similar to gdrnpp_pr's visualize_pose_and_mask.
        """
        bs = 1
        im_H, im_W = int(batch["im_H"][0]), int(batch["im_W"][0])
        image_np = image.copy()  # Original image as numpy (HWC, RGB/BGR)
        ...
        # Get predicted pose and object model
        R_pred = batch["cur_res"][0]["R"]
        t_pred = batch["cur_res"][0]["t"]
        obj_model = self.obj_models[self.obj_id]  # Assumes single obj_id; make dynamic if needed

        # Predicted full mask (from out_dict)
        pred_mask_np = None
        if "full_mask" in out_dict:
            prob = get_out_mask(self.cfg, out_dict["full_mask"])[0, 0]
            prob = torch.unsqueeze(prob, 0)  # (1, Hc, Wc)
            roi = batch["inst_rois"][0, 1:5] if "inst_rois" in batch else batch["roi_center"] - batch["scale"].view(bs, -1) / 2  # Fallback
            pred_mask = paste_masks_in_image(
                prob, roi.view(1, -1), (im_H, im_W), threshold=0.5
            )[0].to(torch.uint8).cpu().numpy()
            pred_mask_np = pred_mask

        # Pose-projected mask from predicted pose
        proj_pts_pred = misc.project_pts(obj_model["pts"], self.cam, R_pred, t_pred)
        mask_pose_pred = misc.points2d_to_mask(proj_pts_pred, im_H, im_W)

        # Create left (predicted mask) and right (predicted pose mask)
        vis_pred_mask = vis_image_mask_cv2(image_np, pred_mask_np, color="red") if pred_mask_np is not None else image_np.copy()
        vis_pred_pose = vis_image_mask_cv2(image_np, mask_pose_pred, color="yellow")

        # Combine left and right horizontally
        combined_vis = cv2.hconcat([vis_pred_mask, vis_pred_pose])

        # If GT pose is provided, add top row with GT visualizations
        if gt_pose is not None:
            # GT mask (projected from GT pose)
            proj_pts_gt = misc.project_pts(obj_model["pts"], self.cam, gt_pose[:3, :3], gt_pose[:3, 3])
            mask_pose_gt = misc.points2d_to_mask(proj_pts_gt, im_H, im_W)
            
            # For GT, we don't have a separate "GT predicted mask" like in gdrnpp_pr, so use the pose mask for both sides
            # (Adapt if you have a GT mask available)
            vis_gt_mask = vis_image_mask_cv2(image_np, mask_pose_gt, color="blue")  # GT pose mask as "mask"
            vis_gt_pose = vis_image_mask_cv2(image_np, mask_pose_gt, color="yellow")  # Same for pose
            
            top_vis = cv2.hconcat([vis_gt_mask, vis_gt_pose])
            # Vertical concatenation: GT on top, predicted below
            combined_vis = cv2.vconcat([top_vis, combined_vis])

        # Save as BGR (OpenCV expects BGR)
        combined_vis_bgr = cv2.cvtColor(combined_vis, cv2.COLOR_RGB2BGR) if combined_vis.shape[-1] == 3 else combined_vis
        save_path = osp.join(results_dir, image_name + ".jpg")
        cv2.imwrite(save_path, combined_vis_bgr)
