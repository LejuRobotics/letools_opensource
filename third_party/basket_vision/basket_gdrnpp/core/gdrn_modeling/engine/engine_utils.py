...
def batch_data(cfg, data, renderer=None, device="cuda", phase="train"):
    ...
    # carry non-tensor identifiers for logging/traceability
    if "scene_im_id" in data[0]:
        batch["scene_im_id"] = [d["scene_im_id"] for d in data]
...
def batch_data_train_online(cfg, data, renderer, device="cuda"):
    ...
    if net_cfg.XYZ_BP:
        pc_cam_tensor = torch.empty((out_res, out_res, 4), device=device, dtype=torch.float32)
        ...
        pc_obj_tensor = torch.empty((out_res, out_res, 4), device=device, dtype=torch.float32)  # xyz
...
def get_renderer(cfg, data_ref, obj_names, gpu_id=None):
    ...
    ren = EGLRenderer(
        ...
        zfar=data_ref.zFar,
        K=data_ref.camera_matrix,  # will override later
...
def vis_batch(cfg, batch, phase="train"):
    ...
    for i in range(n_obj):
        # Print scene/image identifiers to help tracking problematic samples
        if "scene_im_id" in batch:
            print(f"scene_im_id: {batch['scene_im_id'][i]}")
