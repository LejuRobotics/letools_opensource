...
def summary_scores(
    ...
    if len(tabs_col2) == 1:
        ...
        if min_max_thr_str is None:  # not AUC metrics, concat
            ...
            obj_aucs = []
            obj_nums = []
            # choose all rows except header; drop last avg‐row only if there really is one
            rows = (
                tabs_col2[0][1:-1, 0]
                if tabs_col2[0].shape[0] > 2  # header + at least one obj + avg row
                else tabs_col2[0][1:, 0]      # header + exactly one object, no avg row
            )
            for obj_name in rows.tolist():
    ...
    for result_name in tqdm(result_names):
        ...
        if len(big_tab_row) > 0:
            ...
            if len(big_tab_row) == 1:
                ...
                max_cols = max(tab.shape[1] for tab in big_tab_row)
                pieces = []
                for tab in big_tab_row:
                    # keep header for the first, drop it for others
                    block = tab if not pieces else tab[1:, :]
                    # pad to max_cols
                    if block.shape[1] < max_cols:
                        pad = np.full((block.shape[0], max_cols - block.shape[1]), "", dtype=block.dtype)
                        block = np.concatenate([block, pad], axis=1)
                    pieces.append(block)
                res_log_tab = np.concatenate(pieces, axis=0)
