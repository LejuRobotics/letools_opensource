...
class GDRN_Lite(LightningLite):
    ...
    def do_train(self, cfg, args, model, optimizer, renderer=None, resume=False):
        ...
        best_score = float("inf")  # for best model saving
        best_epoch = -1
        ...
        with EventStorage(start_iter) as storage:
            ...
            for iteration in range(start_iter, max_iter):
                ...
                if losses_reduced < best_score:
                    best_score = losses_reduced
                    best_epoch = epoch
                ...
                if iteration - start_iter > 5 and (
                    ...
                    if cfg.TRAIN.VIS_IMG:
                        with torch.no_grad():
                            ...
                            roi_img_vis = denormalize_image(roi_img_vis, cfg).astype("uint8")
                            ...
                            out_mask_vis = out_mask[vis_i].cpu().numpy()
                            tbx_writer.add_image("out_mask", out_mask_vis, iteration)

                            gt_mask_vis = batch["roi_mask_visib"][vis_i].unsqueeze(0).detach().cpu().numpy()
