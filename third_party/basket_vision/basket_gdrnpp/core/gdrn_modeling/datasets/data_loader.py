    ...
    def read_data_train(self, dataset_dict):
        ...
        if self.with_depth:
            if do_replace_bg and self.with_bg_depth:
                mask_bg_depth = (~mask_trunc).astype(np.bool_)
        ...
        # NOTE: full mask
        mask_obj = ((xyz[:, :, 0] != 0) | (xyz[:, :, 1] != 0) | (xyz[:, :, 2] != 0)).astype(np.bool_).astype(np.float32)
...
def build_gdrn_train_loader(cfg, dataset_names):
    ...
    dataset_dicts = filter_invalid_in_dataset_dicts(dataset_dicts, visib_thr=cfg.DATALOADER.FILTER_VISIB_THR) # [:96]
