...
class YCBV_Dataset:
    ...
    def _load_from_idx_file(self, idx_file, image_root):
        ...
        # Scan each scene’s rgb folder for actual frames on disk
        rgb_dirs = sorted(glob.glob(osp.join(image_root, "*/rgb")))
        for rgb_dir in rgb_dirs:
            scene_id = int(osp.basename(osp.dirname(rgb_dir)))
            ...
            # Collect all image IDs from the rgb directory
            for rgb_path in sorted(glob.glob(osp.join(rgb_dir, "*.png"))):
                im_id = int(osp.splitext(osp.basename(rgb_path))[0])
                scene_im_ids.append((scene_id, im_id))
        ...
        for (scene_id, im_id) in tqdm(scene_im_ids):
            ...
            if str_im_id not in scene_gt_dicts[scene_id]:
                # print(f"scene_id: {scene_id}, im_id: {im_id} not in scene_gt_dicts")
                continue
...
update_cfgs = {
    ...
    "ycbv_test": {
        ...
        # "ann_files": [osp.join(DATASETS_ROOT, "BOP_DATASETS/ycbv/keyframe_bop.txt"],
