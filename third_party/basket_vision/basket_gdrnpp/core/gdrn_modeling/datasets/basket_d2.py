...
class Basket_Dataset(object):
    ...
    def __init__(self, data_cfg):
        ...
        self.models_root = data_cfg["models_root"]  # BOP_DATASETS/basket/models
        self.scale_to_meter = data_cfg["scale_to_meter"]  # 1.0
        ...
        self.cat_ids = [cat_id for cat_id, obj_name in ref.basket.id2obj.items() if obj_name in self.objs]
        ...
        scene_dirs = sorted(glob.glob(osp.join(self.dataset_root, '*')))
        self.scenes = [osp.basename(d) for d in scene_dirs]
    ...
    def __call__(self):
        ...
        for scene in tqdm(self.scenes):
            ...
            for str_im_id in tqdm(gt_dict, postfix=f"{scene_id}"):
                ...
                rgb_path = osp.join(scene_root, "rgb", f"{int_im_id:06d}.png")
                assert osp.exists(rgb_path), rgb_path

                depth_path = osp.join(scene_root, "depth", f"{int_im_id:06d}.png")

                scene_im_id = f"{scene_id}/{int_im_id}"

                K = np.array(cam_dict[str_im_id]["cam_K"], dtype=np.float32).reshape(3, 3)
                # 1000 ???
                depth_factor = 1000.0 / cam_dict[str_im_id]["depth_scale"] # raw-mm → meter
    ...
    def models(self):
        ...
        for obj_name in self.objs:
            model = inout.load_ply(
                osp.join(
                    self.models_root,
                    f"obj_{ref.basket.obj2id[obj_name]:06d}.ply",
                ),
...
def get_basket_metadata(obj_names, ref_key):
...
BASKET_OBJECTS = ref.basket.objects

# TODO: add real train
SPLITS_BASKET = dict(
    basket_train=dict(
        name="basket_train",
        dataset_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/basket/train"),
        models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/basket/models"),
        objs=ref.basket.objects,
    ...
    basket_test=dict(
        name="basket_test",
        dataset_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/basket/test"),
        models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/basket/models"),
        objs=ref.basket.objects,
        scale_to_meter=0.001,
        with_masks=True,  # (load masks but may not use it)
...
# single obj splits for basket bop test
for obj in ref.basket.objects:
    for split in [
        "train",
        "test",
    ]:
        # name = "basket_{}_{}".format(obj, split)
        name = f"basket_{split}"
        if name not in SPLITS_BASKET:
            SPLITS_BASKET[name] = dict(
                name=name,
                dataset_root=osp.join(DATASETS_ROOT, f"BOP_DATASETS/basket/{split}"),
                models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/basket/models"),
...
def register_with_name_cfg(name, data_cfg=None):
    ...
    if name in SPLITS_BASKET:
        used_cfg = SPLITS_BASKET[name]
    else:
        assert data_cfg is not None, f"dataset name {name} is not registered"
        used_cfg = data_cfg
    DatasetCatalog.register(name, Basket_Dataset(used_cfg))
    # something like eval_types
    MetadataCatalog.get(name).set(
        id="basket",  # NOTE: for pvnet to determine module
        ref_key=used_cfg["ref_key"],
        objs=used_cfg["objs"],
        eval_error_types=["ad", "rete", "proj"],
        evaluator_type="bop",
        **get_basket_metadata(obj_names=used_cfg["objs"], ref_key=used_cfg["ref_key"]),
    )


def get_available_datasets():
    return list(SPLITS_BASKET.keys())
