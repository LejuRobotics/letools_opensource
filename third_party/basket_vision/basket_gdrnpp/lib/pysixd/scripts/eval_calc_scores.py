...
for error_dir_path in p["error_dir_paths"]:
    ...
    # ====== log teS failures ======
    # if err_type == "teS" and p["correct_th"]["teS"] == [10.0]:
    #     log_file = f"teS_multi_{int(p['correct_th']['teS'][0])}.txt"
    #     with open(log_file, "w") as f:
    #         for m in matches:
    #             # a GT pose is considered *failed* when it is valid
    #             # but **no estimate** was matched (est_id == -1)
    #             # → its error value remains -1
    #             if m["valid"] and m["est_id"] == -1:
    #                 line = (f"{m['scene_id']} {m['im_id']} "
    #                         f"{m['obj_id']} {m['gt_id']} \n")
    #                 f.write(line)
    #                 misc.log("teS fail  scene {:>4}  im {:>6}  obj {}  inst {}".format(m["scene_id"], m["im_id"], m["obj_id"], m["gt_id"]))

    # if False: # err_type == "teS" and p['correct_th']['teS'] == [10.0]:
    #     # log_file = osp.join(
    #     #     p["eval_path"], error_dir_path, "teS_failures.txt"
    #     # )
    #     log_file = f"teS_failures_{int(p['correct_th']['teS'][0])}.txt"
    #     # os.makedirs(osp.dirname(log_file), exist_ok=True)
    #     with open(log_file, "w") as f:
    #         for m in matches:
    #             # valid GT but no matched estimate → failure
    #             # print('haha:', m['error'])
    #             # if m['error'] == -1:
    #             #     pass
    #             if m["valid"] and m["est_id"] == -1:
    #             # if m["valid"] and m['error'] != -1 and m["error"][0] > 2:
    #                 # m["error"][0] is the raw cm‐value
    #                 f.write(f"{m['scene_id']} {m['im_id']} {m['obj_id']} {m['error']}\n")
    #                 misc.log(
    #                     f"teS failure: scene {m['scene_id']} im {m['im_id']} "
    #                     f"obj {m['obj_id']} error={m['error']}cm"
    #                 )
    # ==========================================
