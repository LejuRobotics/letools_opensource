                ...
                for est_id, est in obj_ests_sorted:
                    ...
                    for gt_id, gt in enumerate(scene_gt[im_id]):
                        ...
                        ################################
                            ...
                            te_error = pose_error.te_sym(t_e, t_g, R_gt=R_g, syms=models_sym[obj_id]) / 10
                            e = [te_error]  # mm to cm

                            # if False: # te_error > 10:
                            #     if "rgb_tpath" in dp_split:
                            #         img_path = dp_split["rgb_tpath"].format(scene_id=scene_id, im_id=im_id)
                            #     else:
                            #         img_path = "unknown"
                            #     misc.log(
                            #         "teS failure: scene {} im {} obj {} teS: {:.2f} cm, image: {}".format(
                            #             scene_id, im_id, obj_id, te_error, img_path
                            #         )
                            #     )
                            #     # Append the failure case info to a log file.
                            #     with open("teS_multi_greater_10.txt", "a") as f:
                            #         f.write(
                            #             "{} {} {} {:.2f} {}\n".format(scene_id, im_id, obj_id, te_error, img_path)
                            #         )
