...
对应底层测试: test_kuavo_5w/01_base_control/test_cmd_vel_base.py
适配器接口: LejuWheeledArmHardware.send_base_velocity()
ROS 话题: /cmd_vel (geometry_msgs/Twist)
...
from core.domain.enums import FrameType, MPCControlMode
from core.domain.result import Result


class TestBaseVelocityLocal(unittest.TestCase):
    """底盘速度控制测试类（本体坐标系）"""
    ...
    def setUpClass(cls):
        ...
        # 注意：底盘速度控制 (/cmd_vel) 优先级最高，无需设置 MPC 模式
        # 参考：Kuavo 5-W 接口使用文档.md - "在底盘指令中优先级最高"
        print("\n💡 提示: /cmd_vel 话题优先级最高，无需设置 MPC 模式")
    ...
    def tearDown(self):
        ...
        # 每次测试后停止机器人
        try:
            self.hardware.send_base_velocity(vx=0.0, vy=0.0, vyaw=0.0, frame=FrameType.LOCAL)
            time.sleep(0.5)
    ...
    def test_01_forward_motion(self):
        """测试1: 本体坐标系前进"""
        print("  测试目标: 以 0.3 m/s 的速度前进")
        
        result = self.hardware.send_base_velocity(
            vx=0.3, vy=0.0, vyaw=0.0, frame=FrameType.LOCAL
        )
    ...
    def test_02_backward_motion(self):
        """测试2: 本体坐标系后退"""
        print("  测试目标: 以 0.2 m/s 的速度后退")
        
        result = self.hardware.send_base_velocity(
            vx=-0.2, vy=0.0, vyaw=0.0, frame=FrameType.LOCAL
        )
    ...
    def test_03_lateral_motion_left(self):
        """测试3: 本体坐标系向左平移"""
        print("  测试目标: 以 0.1 m/s 的速度向左平移")
        
        result = self.hardware.send_base_velocity(
            vx=0.0, vy=0.1, vyaw=0.0, frame=FrameType.LOCAL
        )
    ...
    def test_05_rotation_ccw(self):
        ...
        result = self.hardware.send_base_velocity(
            vx=0.0, vy=0.0, vyaw=28.6, frame=FrameType.LOCAL
        )
    ...
    def test_07_combined_motion(self):
        ...
        result = self.hardware.send_base_velocity(
            vx=0.2, vy=0.0, vyaw=17.2, frame=FrameType.LOCAL
        )
    ...
    def test_08_stop_command(self):
        ...
        result = self.hardware.send_base_velocity(
            vx=0.0, vy=0.0, vyaw=0.0, frame=FrameType.LOCAL
        )
...
def run_tests():
    ...
    if not rospy.core.is_initialized():
        rospy.init_node('test_base_velocity_local', anonymous=True)
    
    # 创建测试套件
    suite = unittest.TestLoader().loadTestsFromTestCase(TestBaseVelocityLocal)
...
if __name__ == '__main__':
    print("\n" + "="*70)
    print("Kuavo 5-W 应用层测试 - 底盘速度控制（本体坐标系）")
