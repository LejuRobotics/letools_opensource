...
对应底层测试: test_kuavo_5w/03_arm_control/test_arm_ee_local.py
适配器接口: LejuWheeledArmHardware.send_ee_pose(frame=FrameType.LOCAL)
ROS 话题: /mm/two_arm_hand_pose_cmd (kuavo_msgs/twoArmHandPoseCmd)
反馈话题: /lb_arm_ee_reach_time (std_msgs/Float32)

功能说明:
- 测试局部坐标系（本体坐标系）下的双臂末端位姿控制
- 验证相对位姿变化
- 使用适配器层的 send_ee_pose() 方法
- 自动等待到达时间反馈

注意事项:
1. 源脚本(cmd_arm_ee_local_test.py)不设置 MPC 模式，控制器内部自动管理
2. **必须设置手臂控制模式**：先 Mode 1（重置），再 Mode 2（外部控制）
3. 局部坐标系：基于机器人当前位姿的相对坐标
4. pitch=-90° 是手臂向前伸展的标准姿态，避免奇异点
5. 必须设置 focus_ee=True + focus_z=False
"""
...
class TestArmEELocal(unittest.TestCase):
    """手臂末端位姿控制测试类（局部坐标系）"""
    ...
    def setUpClass(cls):
        ...
        # === 前置设置（与源脚本 cmd_arm_ee_local_test.py 一致）===
        # 1. 躯干复位
        ...
        # 4. 禁用Z轴焦点跟踪（源脚本明确设置）
        cls.hardware.set_focus_z(focus_z=False)
    ...
    def test_01_arms_spread_local(self):
        """测试1: 局部坐标系左右展开"""
        print("  测试目标: 双臂向两侧展开（局部坐标系）")
        ...
        result = self.hardware.send_both_ee_poses(left_pose, right_pose, frame=FrameType.LOCAL)
    ...
    def test_02_arms_forward_local(self):
        """测试2: 局部坐标系前摆臂"""
        print("  测试目标: 双臂向前摆动（局部坐标系）")
    ...
    def test_03_arms_close_local(self):
        """测试3: 局部坐标系前摆臂收拢"""
        print("  测试目标: 双臂向前收拢（局部坐标系）")
    ...
    def test_04_arms_extend_local(self):
        """测试4: 局部坐标系前伸"""
        print("  测试目标: 双臂向前伸展（局部坐标系）")
    ...
    def test_05_arms_lift_local(self):
        """测试5: 局部坐标系抬高"""
        print("  测试目标: 双臂抬高（局部坐标系）")
    ...
    def test_06_arms_return_safe_local(self):
        """测试6: 局部坐标系回到安全位置"""
        print("  测试目标: 回到安全位置（局部坐标系）")
        print("  左手位姿: x=0.5, y=0.2, z=0.85, pitch=-90°")
        print("  右手位姿: x=0.5, y=-0.2, z=0.85, pitch=-90°")
    ...
    def test_07_arms_return_initial_local(self):
        """测试7: 局部坐标系回到初始位置"""
        print("  测试目标: 回到初始姿态（局部坐标系）")
...
def run_tests():
    ...
    if not rospy.core.is_initialized():
        rospy.init_node('test_arm_ee_local', anonymous=True)
    
    # 创建测试套件
    suite = unittest.TestLoader().loadTestsFromTestCase(TestArmEELocal)
...
if __name__ == '__main__':
    print("\n" + "="*70)
    print("Kuavo 5-W 应用层测试 - 手臂末端位姿控制（局部坐标系）")
