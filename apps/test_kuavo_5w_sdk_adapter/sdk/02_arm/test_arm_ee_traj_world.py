"""
手臂末端轨迹测试（世界坐标系，多关键点 + 自动 MPC）

使用的 Adapter 方法: hardware.send_arm_ee_traj_sdk(frame='world')
...
- test_up_trajectory: 双臂末端从默认位姿 (z=0.5m) 向上抬到 (z=0.7m)，持续 3 秒
...
def test_forward_trajectory(hardware):
    ...
    result = hardware.send_arm_ee_traj_sdk(
        left_traj=left_traj, right_traj=right_traj, total_time=3.0, frame='world'
    )
    if result.success:
        logger.info("✅ 前伸轨迹成功")
    else:
...
def test_up_trajectory(hardware):
    """双臂上抬轨迹"""
    logger.info("=== 测试：双臂末端上抬轨迹 (WORLD) ===")
    left_traj = [DEFAULT_POSE, UP_POSE]
    right_traj = [[0.3, -0.25, 0.5, 0, 0, 0, 1], [0.3, -0.25, 0.7, 0, 0, 0, 1]]
    result = hardware.send_arm_ee_traj_sdk(
        left_traj=left_traj, right_traj=right_traj, total_time=3.0, frame='world'
    )
    if result.success:
        ...
        logger.error(f"❌ 上抬轨迹失败: {result.message}")
    time.sleep(4.0)
    return result.success
...
def test_return_trajectory(hardware):
    """返回默认位姿"""
    logger.info("=== 测试：返回默认位姿 (WORLD) ===")
    left_traj = [FORWARD_POSE, DEFAULT_POSE]
    right_traj = [[0.5, -0.25, 0.5, 0, 0, 0, 1], [0.3, -0.25, 0.5, 0, 0, 0, 1]]
    result = hardware.send_arm_ee_traj_sdk(
        left_traj=left_traj, right_traj=right_traj, total_time=3.0, frame='world'
    )
    if result.success:
...
def main():
    hardware = HardwareFactory.create_hardware(config={
        ...
        'sdk_managers_whitelist': ['arm'],# 手臂轨迹只需要 arm 管理器
        'skip_end_effector': True,# 手臂轨迹不需要末端执行器
        'skip_camera': True,# 手臂轨迹不需要相机
        'skip_state_manager': True,# 手臂轨迹不需要状态管理器
        'skip_force_publishers': True,# 手臂轨迹不需要力控发布器
    })
    all_passed = True
    try:
        hardware.initialize()
