"""
底盘位置控制测试（世界坐标系，SDK 直调）

使用的 Adapter 方法: hardware.send_base_position_world_sdk()
底层路径: SDK 直调 → _low_level_sdk_manager.control_base_position → robot_sdk.control.control_base_position

测试用例说明:
- test_move_forward: 底盘在世界坐标系下前进 0.5m（x=+0.5, y=0, yaw=0）
- test_move_lateral: 底盘在世界坐标系下左移 0.3m（x=0, y=+0.3, yaw=0）
- test_rotate: 底盘在世界坐标系下原地旋转 0.5rad（x=0, y=0, yaw=+0.5）
"""
import sys
import time
...
def test_move_forward(hardware):
    """前进 0.5m"""
    logger.info("=== 测试：前进 0.5m (WORLD) ===")
    result = hardware.send_base_position_world_sdk(x=0.5, y=0.0, yaw=0.0)
    if result.success:
        logger.info("✅ 前进成功")
    else:
...
def test_move_lateral(hardware):
    """左移 0.3m"""
    logger.info("=== 测试：左移 0.3m (WORLD) ===")
    result = hardware.send_base_position_world_sdk(x=0.0, y=0.3, yaw=0.0)
    if result.success:
        logger.info("✅ 左移成功")
    else:
...
def test_rotate(hardware):
    """旋转 0.5rad"""
    logger.info("=== 测试：旋转 0.5rad (WORLD) ===")
    result = hardware.send_base_position_world_sdk(x=0.0, y=0.0, yaw=0.5)
    if result.success:
        logger.info("✅ 旋转成功")
    else:
...
def main():
    ...
    try:
        ...
        logger.info("🎉 底盘位置（世界系，SDK 直调）测试完成")
