...
使用的 Adapter 方法: hardware.send_left_arm_joint_timed()
底层路径: TimedCmd → _timed_cmd_manager.send_left_arm_joint → planner_index=8

测试用例说明:
- test_zero: 左臂 7 个关节全部回到 0° 零位，持续 2 秒
- test_forward: 左臂前伸（肩关节 30°），持续 2 秒
- test_up: 左臂上举（肘关节 90°），持续 2 秒
"""
import sys
import time
...
def test_zero(hardware):
    logger.info("=== 测试：左臂零位 ===")
    result = hardware.send_left_arm_joint_timed(joint_angles=ZERO, desire_time=2.0)
    if result.success:
        logger.info("✅ 左臂零位成功")
    else:
        logger.error(f"❌ 左臂零位失败: {result.message}")
    time.sleep(3.0)


def test_forward(hardware):
    logger.info("=== 测试：左臂前伸 ===")
    result = hardware.send_left_arm_joint_timed(joint_angles=FORWARD, desire_time=2.0)
    if result.success:
        logger.info("✅ 左臂前伸成功")
    else:
...
def test_up(hardware):
    logger.info("=== 测试：左臂上举 ===")
    result = hardware.send_left_arm_joint_timed(joint_angles=UP, desire_time=2.0)
    if result.success:
        logger.info("✅ 左臂上举成功")
    else:
        logger.error(f"❌ 左臂上举失败: {result.message}")
    time.sleep(3.0)
